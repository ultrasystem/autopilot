...
#
export X4_BASE		 := $(realpath $(dir $(lastword $(MAKEFILE_LIST))))/
include $(X4_BASE)makefiles/setup.mk
...
#
KNOWN_CONFIGS		:= $(subst config_,,$(basename $(notdir $(wildcard $(X4_MK_DIR)config_*.mk))))
CONFIGS			?= $(KNOWN_CONFIGS)

#
# Boards that we (know how to) build NuttX export kits for.
#
KNOWN_BOARDS		:= $(subst board_,,$(basename $(notdir $(wildcard $(X4_MK_DIR)board_*.mk))))
BOARDS			?= $(KNOWN_BOARDS)
